...
class PointstreakTeamSchedule:
	...
	def init_from_team(self,team):
		self.get_page_source(team.get_url());
		return self;
	def get_page_source(self,url):
		self.htmlSoup = BeautifulSoup(urllib.request.urlopen(url).read(),'html.parser');
		self.get_season_info()
		self.get_game_list();
	def get_season_info(self):
		gameSeason = self.htmlSoup.select(SEASON_SELECTOR)
		if( gameSeason and len(gameSeason) == 1 ):
			pattern = re.compile('[^\d]*(\d{4})([-](\d{4}))?')
	...
	def get_game_list(self):
		...
		for row in gameRows:
			...
			if( len(tds) >= len(ROWS)):
				...
				game.home = tds[0].a.string
				game.away = tds[1].a.string
...
class PointStreakGame:
	...
	def to_calendar_event(self):
		...
		return CalendarEvent.create_event_object(
			event_name = self.home + " vs. " + self.away,
